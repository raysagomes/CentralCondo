...
export default function AvisoCard() {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:3001/notices/${user.entityId}`, {
    ...
    function handleDeleteNotice(id) {
        axios.delete(`http://localhost:3001/notices/${id}`, {
            headers: { Authorization: `Bearer ${user.token}` }
        })
            .then(() => {
                ...
                <>
                    {showInput ? (
                        <div className="aviso-input-group">
                            ...
                            />
                            <div>
                                <button onClick={handleAddNotice} className="aviso-btn save-btn">
                                    Salvar
                                </button>
                                <button onClick={() => setShowInput(false)} className="aviso-btn cancel-btn">
                                    Cancelar
                                </button>
                            </div>
                        </div>
